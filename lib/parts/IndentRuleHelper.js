...
const InfixExpressionIndentValidator = require('./InfixExpressionIndentValidator')

class IndentRuleHelper {
  /**
   * Create meta object.
   * @param {{
   *   description: string,
  ...
  static createRule (context) {
    ...
    /**
     * Get error message if found error.
     * @param {Object} node - AST node.
    ...
    function getErrorMessage (node) {
      ...
      if (differentIndentOfRightOperand(node)) {
        return createErrorMessageOnDifferentIndentOfRightOperand(node)
      }

      const indentValidator = new InfixExpressionIndentValidator(node)

      if (indentValidator.hasShortageIndentBeforeRightOperand()) {
        return createErrorMessageOnShortageIndent(node)
      }

      if (indentValidator.isNotAlignedBothOperands()) {
        return createErrorMessageOnNotAlignedBothOperands(node)
      }
...
module.exports = IndentRuleHelper
