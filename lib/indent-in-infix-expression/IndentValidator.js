...
class IndentValidator {
  ...
  _setupParamsByConstructorArguments () {
    ...
    this.isChoppedDownAfterOperator = this._isChoppedDownAfterOperator()
  ...
  createErrorMessage () {
    ...
    if (
      !this.isChoppedDownBeforeOperator
      && !this.isChoppedDownAfterOperator
    ) {
      return null
    }
  ...
  _createNameOfTargetToFixIndent () {
    return this.isChoppedDownAfterOperator
