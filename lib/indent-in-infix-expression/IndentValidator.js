...
class IndentValidator {
  ...
  /**
   * Fix up.
   * @param {Object} fixer - Instance of Fixer.
   ...
   * @param {Object} targetToken - Target token to remove intent.
   * @returns {boolean} - true: exists.
   */
  _existsInterruptingComment (targetToken) {
    const interruptingToken = this.context
      .getSourceCode()
      .getLastTokenBetween(
        this.node.left,
        targetToken,
    ...
    return interruptingToken?.loc.end.line === targetToken.loc.start.line
  }
}
