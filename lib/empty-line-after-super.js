  ...
  create (context) {
    ...
    return {
      [SUPER_CALL_SELECTOR] (superCallStatement) {
        ...
        const nextTokenStartPosition = getNextTokenStartPosition(nextStatement)

        if (hasEmptyLineAfterSuper(nextTokenStartPosition)) {
          return
        }
         ...
         * @param {Position} nextTokenStartPosition - next statement.
         ...
         *                      otherwise, false.
         */
        function hasEmptyLineAfterSuper (nextTokenStartPosition) {
        ...
        function getSuperStatementTailComment () {
          return context
            ...
            .filter(
              comment => comment.loc.start.line === superCallStatement.loc.end.line
         ...
         * @param {Statement} nextStatement - next statement.
         * @returns {Position} - next token start position.
         */
        function getNextTokenStartPosition (nextStatement) {
          if (!nextStatement) {
            return null
          }
          ...
          const nextToken = comments[0] || nextStatement
        ...
        function getFixingIndent (nextTokenStartPosition) {
          ...
          if (nextTokenStartPosition.line !== superCallStatement.loc.end.line){
            return sourceCode
              .getText()
              .substring(
                sourceCode.getIndexFromLoc({ line: nextTokenStartPosition.line, column: 0 }),
                sourceCode.getIndexFromLoc(nextTokenStartPosition)
              )
          }

          const prevToken = sourceCode.getTokensBefore(
            superCallStatement,
            {
              includeComments: true,
              filter: token => token.loc.start.line === superCallStatement.loc.start.line
