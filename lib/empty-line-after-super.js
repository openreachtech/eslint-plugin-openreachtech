    ...
    return {
      [SUPER_CALL_SELECTOR] (superCallStatement) {
        ...
        context.report({
          ...
          fix (fixer) {
            ...
            const range = [
              ...
              context.sourceCode.getIndexFromLoc(nextTokenStartPosition),
            ]
        ...
        function getFixingIndent (nextTokenStartPosition) {
          const keptSourceCode = context.sourceCode

          if (nextTokenStartPosition.line !== superCallStatement.loc.end.line) {
            return keptSourceCode
              .getText()
              .substring(
                keptSourceCode.getIndexFromLoc({
                  line: nextTokenStartPosition.line,
                  column: 0,
                }),
                keptSourceCode.getIndexFromLoc(nextTokenStartPosition)
              )
          }

          const prevToken = keptSourceCode
            .getTokensBefore(
              superCallStatement,
              {
          ...
          const [end] = (prevToken || superCallStatement).range
